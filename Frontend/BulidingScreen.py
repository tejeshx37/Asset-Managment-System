...
import BuildingScreenDB

def ShowWindow():
    rootBuilding=Tk()
    ob=BuildingScreen(rootBuilding)
    ob.ShowControl()
    rootBuilding.mainloop()
...
class BuildingScreen:
    ...
    def __init__(self,root):
        ...
        self.root=root
        root.title("AMS - Buliding")
        root.geometry("1350x700+0+0")
        root.state(newstate='zoomed')  
 
        title=Label(root,text="BUILDING MASTER SCREEN",font=("times new roman",35,"bold"),bg="darkblue",fg="white")
    ...
    def ShowControl(self):
        ...
        btn_Frame.place(x=85,y=250,width=430)

        # adding add,modify,delete,cancel buttons 
        addbtn=Button(btn_Frame,text="Add",width=10,command=self.add_Building).grid(row=0,column=0,padx=15,pady=10)
        ...
        self.Buliding_table=ttk.Treeview(Table_Frame,columns=("bid","bname","bdesc"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        scroll_x.pack(side=BOTTOM,fill=X)
        scroll_y.pack(side=RIGHT,fill=Y)
        scroll_x.config(command=self.Buliding_table.xview)
        scroll_y.config(command=self.Buliding_table.yview)

        self.Buliding_table.column("bid",anchor=CENTER, stretch=NO, width=70)
        self.Buliding_table.column("bname", anchor=W, stretch=NO, width=200)
        self.Buliding_table.column("bdesc", anchor=W, stretch=NO, width=200)

        self.Buliding_table.heading("bid",text="Building ID")
        self.Buliding_table.heading("bname", text="Building Name")
        self.Buliding_table.heading("bdesc", text="Description")

        self.Buliding_table['show']='headings'
        self.Buliding_table.pack(fill=BOTH,expand=1)
        self.Buliding_table.bind("<ButtonRelease-1>",self.get_cursor)
        self.fetch_data()
       

    def get_cursor(self,ev):
        curosor_row=self.Buliding_table.focus()
        contents=self.Buliding_table.item(curosor_row)
        row=contents['values']
    ...
    def search_data(self):
        ...
        icnt = int()
        icnt=0
        rows=BuildingScreenDB.getSearchRecord(self.search_by.get().strip(),self.search_txt.get().strip())
        if len(rows)!=0:
            self.Buliding_table.delete(*self.Buliding_table.get_children()) 
            for row in rows:
                self.Buliding_table.insert('',END,values=row) 
                icnt+=1
    ...
    def fetch_data(self):
        icnt = int()
        icnt=0
        rows=BuildingScreenDB.getRecords()
        if len(rows)!=0:
            self.Buliding_table.delete(*self.Buliding_table.get_children()) 
            for row in rows:
                self.Buliding_table.insert('',END,values=row)
    ...
    def add_Building(self):
        if self.IsEmpty():
            return
        bResult=BuildingScreenDB.IsbuildExist(self.txtname.get().strip())
        if(bResult):
        ...
        bInserted=BuildingScreenDB.addnewrecord(self.txtname.get().strip(),self.txtdescr.get().strip())
        if bInserted:
            self.Clear_Building()
            self.fetch_data()
    ...
    def Modify_Building(self):
        ...
        bUpdated=BuildingScreenDB.UpdateBuilding(self.txtid.get().strip(),self.txtname.get().strip(),self.txtdescr.get().strip())
        if bUpdated:
            self.Clear_Building()
            self.fetch_data()
    ...
    def Delete_Building(self):
        ...
        if answer:         
            bDeleted=BuildingScreenDB.DeleteBuilding(self.txtid.get())
            # messagebox.showinfo(title="AMS",message=self.txtid.get()+self.txtname.get()+self.txtdescr.get())
            if bDeleted:
                self.Clear_Building()
                self.fetch_data()
    ...
    def add(a,c):
        bInserted=BuildingScreenDB.addnewrecord(a,c)
        if bInserted:
            messagebox.showinfo(title="AMS",message="Record sucessfuly added.")
        else:
            messagebox.showerror(title="AMS",message="Record not saved.")
